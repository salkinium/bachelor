...
def plot_values(ax, values, color='k'):
    plot = ax.plot(values['temperature'], values['values'], '{}-'.format(color),
                   values['temperature'], values['values'], '{}o'.format(color), markersize=5)
...
if __name__ == "__main__":
    ...
    ax.axhline(5, linestyle='--', color='0.66', linewidth=2, zorder=1)
    ax.axhline(-5, linestyle='--', color='0.66', linewidth=2, zorder=1)
    ax.set_axisbelow(True)

    l = pylab.legend([b4800_line, b19200_line, b115200_line, b57600_line],
                 ["4800 Baud", "19200 Baud", "115200 Baud", "57600 Baud"],
                 prop = {'size': 11})
    l.set_zorder(10)

    pylab.savefig(os.path.join(os.path.dirname(__file__), "{}_baudrate_error.pdf".format(baudrate_file)), bbox_inches='tight', pad_inches=0.1)
    print (-0.364828571429 - 0.36455 - 0.361257142857 - 0.378014285714) / 4.0

    # plot dco error
    fig2, ax2 = plt.subplots(1)
    fig2.set_size_inches(8 * 0.625, 0.625 * 1 * 5.5)
    ...
    ax2.set_xlabel("Temperature ($^\circ$C)", fontsize=18)
    ax2.grid(b=True, which='major', linestyle='-')
    ax2.axhline(5, linestyle='--', color='0.66', linewidth=2, zorder=1)
    ax2.axhline(-5, linestyle='--', color='0.66', linewidth=2, zorder=1)
    ax2.set_axisbelow(True)

    pylab.savefig(os.path.join(os.path.dirname(__file__), "{}_dco_error.pdf".format(reboot_file)), bbox_inches='tight', pad_inches=0.1)

    # plot inverse table
    fig3, ax3 = plt.subplots(1)
    fig3.set_size_inches(8 * 0.625, 0.625 * 1 * 5.5)
    ...
    ax3.set_ylabel("Baudrate (kBaud)", fontsize=18)
    ax3.set_xlabel("Temperature ($^\circ$C)", fontsize=18)
    ax3.grid(b=True, which='major', linestyle='-')
    ax3.axhline(115.200, linestyle='-', color='0.66', linewidth=1.5, zorder=1)
    ax3.set_axisbelow(True)

    pylab.savefig(os.path.join(os.path.dirname(__file__), "{}_table.pdf".format(baudrate_file)), bbox_inches='tight', pad_inches=0.1)

    # plot correction error
    fig4, ax4 = plt.subplots(1)
    fig4.set_size_inches(8 * 0.625, 0.625 * 1 * 5.5)
    ...
    ax4.set_ylabel("Relative Error (%)", fontsize=18)
    ax4.set_xlabel("Temperature ($^\circ$C)", fontsize=18)
    ax4.grid(b=True, which='major', linestyle='-')
    ax4.axhline(5, linestyle='--', color='0.66', linewidth=2, zorder=1)
    ax4.axhline(-5, linestyle='--', color='0.66', linewidth=2, zorder=1)
    ...
    pylab.savefig(os.path.join(os.path.dirname(__file__), "{}_correction_error.pdf".format(correction_file)), bbox_inches='tight', pad_inches=0.1)
